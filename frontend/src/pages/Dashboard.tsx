...
import { MOCK_DASHBOARD_STATS, MOCK_DASHBOARD_ALERTS } from '../mock/dashboard';
...
export default function Dashboard() {
  ...
  const { totalDrivers, totalVehicles, activeRegistrations, pendingViolations } = MOCK_DASHBOARD_STATS;
  const { expiredLicenseDrivers, expiredRegistrationVehicles, approachingSuspensionDrivers } = MOCK_DASHBOARD_ALERTS;
              ...
              <ul className="divide-y divide-border">
                {approachingSuspensionDrivers.map((d) => (
                  <li key={d.license_number} className="px-5 py-3 flex items-center justify-between gap-3">
                    <div className="flex flex-col gap-0.5 min-w-0">
                      <Link
                        to={buildRoute.driverProfile(d.license_number)}
                        className="text-sm font-medium text-brand-600 hover:underline truncate"
                      >
                        {d.full_name}
                      </Link>
