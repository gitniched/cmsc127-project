...
The following queries are at the bottom of the SQL file. They use session variables as parameters (set by the calling application before executing).

**1. Filter drivers**
```sql
SELECT * FROM v_driver
WHERE (@license_type IS NULL OR license_type = @license_type)
  AND (@license_status IS NULL OR license_status = @license_status)
  AND (@age_min IS NULL OR @age_max IS NULL OR age BETWEEN @age_min AND @age_max)
  AND (@sex IS NULL OR sex = @sex);
```
Any filter can be `NULL` to skip it. Uses `v_driver` so `age` is always computed live.
...
```sql
-- Set @license_number, @start_date, @end_date before running
SELECT tv.uovr_number, tv.officer, tv.violation_status, tv.violation_location_city,
       tv.violation_location_region, tv.violation_date, tv.payment_status,
       tv.plate_number, tv.registration_number, vt.violation_type
...
```
Because of the JOIN to `violation_type`, incidents with multiple violation types return one row per type. Group by `uovr_number` in the application layer if you need one row per incident.

**6. Total violations per type for a given year**
```sql
-- Set @year before running
...
```sql
-- Set @city and/or @region before running
...
**8. All vehicles per driver**
```sql
SELECT d.license_number, d.license_type,
...
```
Uses `LEFT JOIN` so drivers with no registered vehicles still appear.

**9. Registration history per vehicle**
```sql
-- Set @plate_number, or leave NULL to return all vehicles
SELECT v.plate_number, v.make, v.model,
       vr.registration_number, vr.registration_date,
